...
//
// Ported to OpenSSL 1.1+/3.x: BIGNUM became an opaque type, so CBigNum now
// wraps a BIGNUM* instead of inheriting from BIGNUM. Identical semantics.
...
public:
    ...
    CBigNum(char n)             { init(); if (n >= 0) setulong(n); else setint64(n); }
